  ...
  <div class="main" v-loading="loading">
    <el-container style="height: 100%">
      <el-header>
        <el-row>
          <el-col :span="2">
            <el-select v-model="groupName" placeholder="请选择" title="选择分组" style="width: 120px;"
                       @change="groupChanged">
              <el-option
  ...
  export default {
    ...
    methods: {
      ...
      handleOpenItem(key, keyPath) {
        let keys = key.split('-');
        let func = this.newTags[keys[0]].items[keys[1]];
        ...
        for (let prop in module.properties) {
          if (module.properties.hasOwnProperty(prop)) {
            ...
            if (property.$ref !== undefined) {
              property.originalRef = property.$ref.substring('#/definitions/'.length)
            }
            if (property.originalRef !== undefined) {
              if (property.originalRef === 'Timestamp') {
                property.type = 'number'
              } else {
                this.parseModule(this.form.definitions[property.originalRef], modules);
                property.type = this.form.definitions[property.originalRef].title;
              }
            } else if (property.type === 'array') {
              if (property.items.$ref !== undefined) {
                property.items.originalRef = property.items.$ref.substring('#/definitions/'.length);
              }
              if (property.items.originalRef !== undefined) {
                this.parseModule(this.form.definitions[property.items.originalRef], modules);
              } else if (property.items.type === 'array') {
                if (property.items.items.$ref !== undefined) {
                  property.items.items.originalRef = property.items.items.$ref.substring('#/definitions/'.length);
                }
                this.parseModule(this.form.definitions[property.items.items.originalRef], modules);
              }
            }
          }
        }
